...
//		System.out.println(name);
	...
	public boolean startFight(String protagName)
	{
...
//	    allFighters.get(1).setTerrain(t);
	      ...
	      //asks players for what actions to preform
	    	  output.displayString("Current Teams:\n", 0);
	    		System.out.println("Current Teams:\n");
		    	for (int i = 0; i < teams.size(); i++)
		    	{
		    		 output.displayString(teams.get(i).toString(), 0);
		    	}
...
//		        System.out.println();
	      ...
	      for (int i = allFighters.size() - 1; i >= 0; i--) {
	        if (allFighters.get(i).isHealthZero(log, this)) {
	          ...
	          if (protagName != null
	        		  && target.getName().equals(protagName))
	          {
	        	  return true;
	          }
			    output.displayString(target.getName() + " was eliminated. ", countOfSpawn);
	        }
	      }
	    ...
	    ArrayList<Entity> members = teams.get(0).getMembers();
	      output.displayString(teams.get(0).getAllMembers() + " won!", countOfSpawn);
	     ...
	     for (int i = 0; i < members.size(); i++)
	    {
	    	if (members.get(i).getName().equals(protagName))
	    	{
	    		return false;
	    	}
	    }
