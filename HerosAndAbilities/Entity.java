...
public class Entity implements Comparable<Entity> {
  ...
  public static enum Statistic {
    SPEED, MAX_HEALTH, HEALTH, BASE_ATTACK, BASE_DEFENSE, SHIELD
  }

  public Entity(String name, int speed, int health, int shieldHealth, Entity creator) {
    this.name = name;
    this.speed = speed;
    this.health = health;
  ...
  public String toString() {
    StringBuilder EntityString = new StringBuilder(name).append(" - will of ").append(speed).append(". \n")
        .append("\u001B[35m");
    if (abilities.size() == 0) {
      ...
      for (int i = 0; i < abilities.size(); i++) {
        ...
        EntityString.append("* ").append(a.toString());
        if (!a.ableToUseAbility()) {
          EntityString.append(" (on cooldown. needs ").append(a.getTurnsNeeded()).append(" more turns)");
        }
        EntityString.append("\n");
      }
    }
    ...
    EntityString.append("The state is: ").append(state.toString()).append("\n").append("\u001B[31m");
    if(effects==null){
    ...
    if (effects.size() == 0) {
      ...
      for (int i = 0; i < effects.size(); i++) {
        EntityString.append("* ").append(effects.get(i)).append("\n");
      }
    }
    ...
    if (shields.size() == 0) {
      ...
      for (int i = 0; i < shields.size(); i++) {
        EntityString.append("* ").append(shields.get(i)).append("\n");
      }
    }

    EntityString.append("\u001B[34m").append("* health - ").append(health).append("/").append(maxHealth).append("\n")
        .append("* shield - ").append(shieldHealth).append("\n").append("* base attack - ").append(baseAttack)
        .append("\n").append("* base defense - ").append(baseDefense).append("\n").append("\u001B[0m").append("\n");
  ...
  public Ability getAbility(Abilities.Name enumName) {
  ...
  public boolean hasGroupAbility(Abilities.Name name) {
   ...
   */
  public boolean dealDamage(int damageDealt, boolean isPiercing, boolean ignoresDefense, Entity caster, Element e) {
  ...
  public boolean dealEffectDamage(int damageDealt, boolean isPiercing, boolean ignoresDefense) {
    if (!ignoresDefense) {
      damageDealt -= baseDefense;
    }
  ...
  public void removeEffects(Elements.Name elementID) {
    for (int i = effects.size() - 1; i >= 0; i--) {
      Effect e = effects.get(i);
      if (e.isRemovable() && (elementID.equals(Elements.Name.ALL) || elementID.equals(e.getElement().getID()))) {
        removeEffect(e);
      }
    }
  }
  ...
  public boolean searchForShield(Shields.Trigger trigger, Element element, Entity target, Entity caster) {
   ...
   * Getting an action
   */
  public List<Action> getActions(List<Entity> allHeros, InputSystem inputReader) {
